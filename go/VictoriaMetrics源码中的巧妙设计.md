...
see： [victoria-metrics-1.72.0/blob/master/VictoriaMetrics-1.72.0-cluster/lib/cgroup/cpu.go](https://link.zhihu.com/?target=https%3A//github.com/ahfuzhang/victoria-metrics-1.72.0/blob/master/VictoriaMetrics-1.72.0-cluster/lib/cgroup/cpu.go)
...
see: [VictoriaMetrics-1.72.0-cluster/app/vmstorage/transport/server.go#L261](https://link.zhihu.com/?target=https%3A//github.com/ahfuzhang/victoria-metrics-1.72.0/blob/63987bc868a6983bc94192ed24136590016575c8/VictoriaMetrics-1.72.0-cluster/app/vmstorage/transport/server.go%23L261)
...
具体的细节请看我的这篇分析文章：《 [VictoriaMetrics中协程优先级的处理方式](https://link.zhihu.com/?target=https%3A//www.cnblogs.com/ahfuzhang/p/15847860.html) 》
...
see: [/VictoriaMetrics-1.72.0-cluster/lib/cgroup/mem.go](https://link.zhihu.com/?target=https%3A//github.com/ahfuzhang/victoria-metrics-1.72.0/blob/master/VictoriaMetrics-1.72.0-cluster/lib/cgroup/mem.go)
...
相比之下，prometheus和 [thanos](https://zhida.zhihu.com/search?content_id=192406479&content_type=Article&match_order=1&q=thanos&zhida_source=entity) 在请求量大的情况下极易发生OOM崩溃。
...
关于fastcache组件，请看我的这篇分析：《 [介绍一个golang库：fastcache](https://link.zhihu.com/?target=https%3A//www.cnblogs.com/ahfuzhang/p/15840313.html) 》
...
中型对象的分配和释放相对不是很频繁，因此使用sync.Pool来作为对象池，就可以重用这些对象。 中型对象提供了reset()方法，把缓冲区的开始位置置0，而不是解除引用。中型对象相关的所有成员都只会分配一次，然后不再释放。
...
关于ZSTD的介绍，请看我的这篇帖子：《 [介绍一个golang库：zstd](https://link.zhihu.com/?target=https%3A//www.cnblogs.com/ahfuzhang/p/15842350.html) 》
...
see: [VictoriaMetrics-1.72.0-cluster/lib/storage/storage.go](https://link.zhihu.com/?target=https%3A//github.com/ahfuzhang/victoria-metrics-1.72.0/blob/63987bc868a6983bc94192ed24136590016575c8/VictoriaMetrics-1.72.0-cluster/lib/storage/storage.go%23L509)
...
请移步到我的这篇文章：《 [vm中仿照RoaringBitmap的实现：uint64set](https://link.zhihu.com/?target=https%3A//www.cnblogs.com/ahfuzhang/p/15900852.html) 》

## 位运算技巧

请移步到我的这篇文章：《 [如何计算一个uint64类型的二进制值的尾部有多少个0](https://link.zhihu.com/?target=https%3A//www.cnblogs.com/ahfuzhang/p/15900551.html) 》

## if语句上的string()转换会被编译器优化

请移步到我的这篇文章：《 [golang的if比较中的string转换会被编译器优化](https://link.zhihu.com/?target=https%3A//www.cnblogs.com/ahfuzhang/p/15879423.html) 》
...
> ——《 [Go 语言高性能编程](https://link.zhihu.com/?target=https%3A//geektutu.com/post/hpg-range.html) 》
...
> 通常有两种不同的加速查询处理的方法：矢量化查询执行和运行时代码生成。在后者中，动态地为每一类查询生成代码，消除了间接分派和动态分派。这两种方法中，并没有哪一种严格地比另一种好。运行时代码生成可以更好地将多个操作融合在一起，从而充分利用 CPU 执行单元和流水线。矢量化查询执行不是特别实用，因为它涉及必须写到缓存并读回的临时向量。如果 L2 缓存容纳不下临时数据，那么这将成为一个问题。但矢量化查询执行更容易利用 CPU 的 SIMD 功能。朋友写的一篇研究论文表明，将两种方法结合起来是更好的选择。 [ClickHouse](https://zhida.zhihu.com/search?content_id=192406479&content_type=Article&match_order=1&q=ClickHouse&zhida_source=entity) 使用了矢量化查询执行，同时初步提供了有限的运行时动态代码生成。

从实现带来来看，VictoriaMetrics并未采用SIMD和JIT这两项技术。可以期待，未来VM的性能还能更高！

我已将注释版的源码放在了这里： [github.com/ahfuzhang/vi](https://link.zhihu.com/?target=https%3A//github.com/ahfuzhang/victoria-metrics-1.72.0) ，对存储引擎实现有兴趣的朋友可以来一起注释。
